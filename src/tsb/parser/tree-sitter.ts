...
// Built once via `cd zed/tree-sitter-tsb && tree-sitter build --wasm`.
const WASM_PATH = resolve(HERE, "../../../zed/tree-sitter-tsb/tree-sitter-tsb.wasm");
...
export async function parseToAst(source: string): Promise<SourceFileNode> {
  ...
  if (!tree) {
    throw new Error("tsb parse failed: tree-sitter returned no tree");
  }
  const ast = nodeToAst(tree.rootNode);
  tree.delete();
  if (ast.kind !== "source_file") {
    throw new Error(`tsb parse: expected source_file at root, got ${ast.kind}`);
  }
  return ast;
}
