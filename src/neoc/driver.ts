/**
 * One-shot transpile entry point. Reads a `.neoc` file, runs the rewriter,
 ...
 * `neoc.d.ts` (ambient types + function signatures) and
 * `neoc.runtime.ts` (the runtime that installs the functions on
...
export interface CompileOptions {
  ...
  input: string;
  /** Optional output path. Defaults to the input with `.neoc` swapped for `.ts`. */
  output?: string;
  /** Absolute or dynamically importable paths for user-authored macros. */
  macroModules?: string[];
  /**
   * Directory to write the shared `neoc.d.ts` + `neoc.runtime.ts`
...
export interface CompileResult {
  ...
  /** True if the neoc runtime artefacts were referenced. */
  usesResult: boolean;
}
...
export async function compileFile(opts: CompileOptions): Promise<CompileResult> {
  ...
  // Side-effect import that loads the neoc runtime (installs globalThis
...
/**
 * `foo/bar/baz.neoc` -> `foo/bar/baz.ts`. Any other extension is preserved
 * as a sibling `.ts` (e.g. `baz.neoc.neoc` -> `baz.neoc.ts`).
 */
function defaultOutputPath(input: string): string {
  if (input.endsWith(".neoc")) return `${input.slice(0, -4)}.ts`;
...
export async function buildProject(opts: BuildOptions): Promise<string[]> {
  ...
  if (anyUsesResult) {
    ...
    written.push(path.join(opts.cwd, "neoc.d.ts"));
    written.push(path.join(opts.cwd, "neoc.runtime.ts"));
  }
...
/**
 * Resolve a list of source globs (relative to `cwd`) into absolute `.neoc`
 ...
 * `neoc.runtime.ts` at the build root, and prepend a side-effect
...
function prependRuntimeImport(
  ...
  const importPath = `${normalised.replace(/\\/g, "/")}/neoc.runtime.ts`;
  return `import ${JSON.stringify(importPath)};\n${ts}`;
}

/**
 * Write the shared `neoc.d.ts` + `neoc.runtime.ts` artefacts at the
 * build root. `neoc.d.ts` declares ambient types + function
 * signatures so compiled `.ts` files don't need explicit imports;
 * `neoc.runtime.ts` installs the runtime on `globalThis` when its
 * side-effect import runs.
 */
async function writeBunnyArtefacts(buildRoot: string): Promise<void> {
  await Bun.write(path.join(buildRoot, "neoc.d.ts"), NEOC_DTS);
  await Bun.write(path.join(buildRoot, "neoc.runtime.ts"), NEOC_RUNTIME);
}

const NEOC_DTS = `// AUTO-GENERATED by neoc. Provides ambient types and function
// signatures referenced by compiled .ts files. The matching runtime
// lives in \`neoc.runtime.ts\` and is loaded via a side-effect import
...
const NEOC_RUNTIME = `// AUTO-GENERATED by neoc. Installs the runtime globals declared in
// \`neoc.d.ts\`. Compiled .ts files load this via a side-effect import
