...
class NeocLexer : LexerBase() {
  ...
  override fun advance() {
    ...
    if (c.isWhitespace()) {
      ...
      tokenType = NeocTokenTypes.WHITESPACE
      return
    }
    ...
    if (c == '/') {
      ...
      if (next == '/') {
        ...
        tokenType = if (docStyle) NeocTokenTypes.DOC_LINE_COMMENT else NeocTokenTypes.LINE_COMMENT
        return
      }
      if (next == '*') {
        ...
        tokenType = if (docStyle) NeocTokenTypes.DOC_BLOCK_COMMENT else NeocTokenTypes.BLOCK_COMMENT
        return
      }
    }
    ...
    if (c == '#' && peek(1) == '[') {
      ...
      tokenType = NeocTokenTypes.ATTRIBUTE
      return
    }

    if (c == '"' || c == '\'') {
      endOffset = scanQuoted(c)
      tokenType = NeocTokenTypes.STRING
      return
    }
    if (c == '`') {
      endOffset = scanQuoted('`')
      tokenType = NeocTokenTypes.TEMPLATE_STRING
      return
    }
    ...
    if (c.isDigit()) {
      ...
      tokenType = NeocTokenTypes.NUMBER
      return
    }
    ...
    if (c.isLetter() || c == '_' || c == '$') {
      ...
      tokenType = when {
        word in KEYWORDS -> NeocTokenTypes.KEYWORD
        word in TYPE_KEYWORDS -> NeocTokenTypes.TYPE_KEYWORD
        word.isNotEmpty() && word[0].isUpperCase() -> NeocTokenTypes.TYPE_IDENTIFIER
        precededByDot && followedByParen -> NeocTokenTypes.METHOD
        precededByDot -> NeocTokenTypes.PROPERTY
        followedByParen -> NeocTokenTypes.FUNCTION
        else -> NeocTokenTypes.IDENTIFIER
      }
      return
    }
    ...
    if (c in PUNCT_CHARS) {
      endOffset = startOffset + 1
      tokenType = NeocTokenTypes.PUNCTUATION
      return
    }
    ...
    if (c in OPERATOR_CHARS) {
      ...
      tokenType = NeocTokenTypes.OPERATOR
      return
    }

    endOffset = startOffset + 1
    tokenType = NeocTokenTypes.BAD_CHARACTER
  }
  ...
  private fun isTrivia(t: IElementType): Boolean =
    t == NeocTokenTypes.WHITESPACE
      || t == NeocTokenTypes.LINE_COMMENT
      || t == NeocTokenTypes.DOC_LINE_COMMENT
      || t == NeocTokenTypes.BLOCK_COMMENT
      || t == NeocTokenTypes.DOC_BLOCK_COMMENT
  ...
  companion object {
    private val KEYWORDS = setOf(
      ...
      // Lua literals + operators (the latter as identifiers — `and` /
      // `or` / `not` are lexed as words, not operator chars)
