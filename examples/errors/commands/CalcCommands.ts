...
// Render an error as a human-readable string. `match` patterns can
// both *check* (`kind: "X"` requires the field to equal the literal)
// and *bind* (`input: s` binds the field's value to a new identifier
// for the arm body). Mixing both keeps the arm self-contained — no
// need to reach back through the outer `err` to pull fields out.
function describe(err: CalcError): string {
  return ((__m) => {
  if (typeof __m === "object" && __m !== null && (__m as Record<string, unknown>).kind === "BadNumber") { const s = (__m as any).input; return `not a number: ${s}`; }
  if (typeof __m === "object" && __m !== null && (__m as Record<string, unknown>).kind === "UnknownOp") { const o = (__m as any).op; return `unknown operator: ${o}; supported: + - * /`; }
